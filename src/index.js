...
function getAppHTML() {
  ...
  <div class="dashboard">
      <div class="settings-grid no-print">
          <div class="panel">
              ...
              <input type="text" id="inputText" value="ABHISHEK" style="width:100%; margin-bottom:10px; font-weight:bold; letter-spacing:1px;">
  ...
  // Smoothed Vector Skeletons for Realistic Letters
  const SKELETONS = {
  ...
  function calculateVectorCAD() {
      ...
      for (let i = 0; i < text.length; i++) {
          ...
          segments.forEach(seg => {
              ...
              offsetPxArr.forEach(offset => {
                  for (let j = 0; j < seg.length - 1; j++) {
                      ...
                      let startMargin = (len - (steps * spacingPx)) / 2;

                      for(let s = 0; s <= steps; s++) {
                          let d = startMargin + s * spacingPx;
                          let px = ox1 + (dx/len) * d;
                          let py = oy1 + (dy/len) * d;
                          rawPoints.push({ x: px, y: py, angle: angle });
                      }
                  }
              });
          });

          // 🚨 THE ULTIMATE ANTI-OVERLAP FILTER 🚨
          // यह सुनिश्चित करता है कि कोनों (Intersections) पर दो LED कभी नहीं भिड़ेंगी
          let cleanPoints = [];
          rawPoints.forEach(p1 => {
              // 85% Strictness: अगर कोई LED पहले से मौजूद LED के 85% Spacing के अंदर है, तो उसे हटा दो!
              let isDup = cleanPoints.some(p2 => Math.hypot(p2.x - p1.x, p2.y - p1.y) < spacingPx * 0.85);
          ...
          currentX += (maxX * scaleBase) + (spacingIn * PPI * 2);
      }
